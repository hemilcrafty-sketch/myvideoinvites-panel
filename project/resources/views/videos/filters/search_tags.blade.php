                ...
                <div class="row justify-content-between flex-wrap">
                    ...
                    <div class="col-md-8 col-12">
                        @include('partials.filter_form', [
                            'action' => route('show_video_search_tag'),
                        ])
                    </div>
                </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modal_title">Add Video Search Tag</h5>
                ...
                <form method="post" id="submit_form" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="itemName" name="name"
                            placeholder="Video Search Tag Name" required />
                    </div>
...
<script>
    function openAddModal() {
        $('#modal_title').text('Add Video Search Tag');
        $('#submit_btn').text('Save');
        $('#submit_form')[0].reset();
        $('#item_id').val('');
        $('#assignSubCatSelect').val('');
        $('#add_modal').modal('show');
    }

    function openEditModal(id, name, status, id_name, seo_emp_id) {
        $('#modal_title').text('Edit Video Search Tag');
        $('#submit_btn').text('Update');
        $('#item_id').val(id);
        $('#itemName').val(name);
        $('#itemStatus').val(status);
        $('#itemIDName').val(id_name);
        $('#assignSubCatSelect').val(seo_emp_id || '');
        $('#add_modal').modal('show');
    }
    ...
    function delete_click(id) {
        ...
        var url = "{{ route('video_search_tag.delete', ':id') }}";
