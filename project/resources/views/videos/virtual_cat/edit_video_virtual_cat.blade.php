...
<div class="main-container seo-all-container">
        ...
        <div class="min-height-200px">
            ...
            <div class="pd-20 card-box mb-30">
                ...
                <form method="post" id="dynamic_form" enctype="multipart/form-data" class="{{ request('preview') ? 'preview-mode' : '' }}">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input class="form-control" type="text" name="meta_title" id="meta_title"
                                    ...
                                    oninput="updateCount(this, 'metaCounter')">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input class="form-control" type="text" name="h1_tag" id="h1_tag" maxlength="60"
                                    ...
                                    required
                                    oninput="updateCount(this, 'h1Counter')">
                    ...
                    <input type="hidden" name="generatedQuery" id="generatedQuery" value="">
    ...
    <script>
        ...
        if (typeof jQuery !== 'undefined') {
            jQuery(document).ready(function() {
                ...
                fields.forEach(field => {
                    ...
                    if (el) updateCount(el, field.counter);
                });
                    ...
                    jQuery.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (data.error) {
                                ...
                                window.alert(data.success || 'Category updated successfully.');
                                window.location.href = "{{ route('show_video_virtual_cat') }}";
                            }
                        },
                        error: function(error) {
                            jQuery('#main_loading_screen').hide();
                            window.alert('Submission failed: ' + error.responseText);
                        },
                        cache: false,
                        contentType: false,
                        processData: false
                ...
                if ("{{ request('preview') }}" == "1") {
                    jQuery('input, textarea, select').prop('disabled', true);
                    jQuery('.add-content-btn, .remove_block, .add_faq_btn, .remove_faq, .dynamic-file').hide();
                    jQuery('.submit-btn').hide();
                    jQuery('button:contains("Add Content"), button:contains("Add Faqs")').hide();
                    jQuery('.btn-success:contains("Edit"), .btn-danger:contains("delete")').hide();
                    jQuery('.btn-success:contains("Add tag"), .btn-danger:contains("Remove Content")').hide();
                    jQuery('#parentCategoryInput').css('pointer-events', 'none').css('background-color', '#f8f9fa');
                    jQuery('<style>')
                        .prop('type', 'text/css')
        ...
        function openRejectModal(id) {
            jQuery('#reject_task_id').val(id);
            jQuery('#rejectModal').modal('show');
        }

        function rejectTask() {
            let id = jQuery('#reject_task_id').val();
            let reason = jQuery('#reject_reason').val();
