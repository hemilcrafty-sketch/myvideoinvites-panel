                ...
                <form id="editVideoForm" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        @if($dataArray['item']->video_thumb)
                            <div style="margin-top: 10px;">
                                ...
                                <div
                                    style="display: none; padding: 10px; background: #f8d7da; border-radius: 6px; color: #721c24;">
                                    <i class="fa fa-exclamation-triangle"></i> Image not found
                                </div>
                                <p style="font-size: 12px; color: #6c757d; margin-top: 5px;">Current:
                                    {{ basename($dataArray['item']->video_thumb) }}</p>
                            </div>
                        @endif
                    </div>
                            ...
                            <div style="margin-top: 10px;">
                                ...
                                @else
                                    <video width="200" height="150" controls
                                        ...
                                        <source src="{{ config('filesystems.storage_url') }}{{ $dataArray['item']->video_url }}"
                                            type="video/{{ $extension }}">
                                        Your browser does not support the video tag.
                                    </video>
                                @endif
                                <p style="font-size: 12px; color: #6c757d; margin-top: 5px;">Current:
                                    {{ basename($dataArray['item']->video_url) }}</p>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        @if($dataArray['item']->video_zip_url)
                            <div
                                style="margin-top: 10px; padding: 10px; background: #f8f9fa; border-radius: 6px; border: 1px solid #e0e0e0;">
                                <i class="fa fa-file-archive-o" style="color: #0059b2; margin-right: 8px;"></i>
                                <span
                                    style="font-size: 13px; color: #4a4a4a;">{{ basename($dataArray['item']->video_zip_url) }}</span>
                            </div>
                        @endif
                    </div>
                        ...
                        <div class="col-md-2 col-sm-12">
                            <div class="form-group">
                                ...
                                <input class="form-control" id="watermark_height" type="number"
                                    value="{{ $dataArray['item']->watermark_height }}" name="watermark_height" required>
                            </div>
                        </div>
                        <div class="col-md-2 col-sm-12">
                            <div class="form-group">
                            ...
                            @for ($i = 0; $i < count($dataArray['editable_image']); $i++)
                                <div class="row">
                                    ...
                                    <div class="col-md-4 col-sm-12">
                                        <div class="form-group">
                                            <h6>Id</h6>
                                            <input type="text" class="form-control" placeholder="key" id="img_key"
                                                name="img_key[]" value="{{ $dataArray['editable_image'][$i]['key'] }}"
                                                required>
                                        </div>
                                    </div>
                                    <div class="col-md-4 col-sm-12">
                            ...
                            @for ($i = 0; $i < count($dataArray['editable_text']); $i++)
                                <div class="row">
                                    ...
                                    <div class="col-md-2 col-sm-12">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" placeholder="Title" id="title"
                                                name="title[]" value="{{ $dataArray['editable_text'][$i]['title'] }}"
                                                required>
                                        </div>
                                    </div>
                                    <div class="col-md-2 col-sm-12">
                                    ...
                                    <div class="col-md-4 col-sm-12">
                                        <div class="form-group">
                                            <h6>Value</h6>
                                            <textarea style="height: 80px" class="form-control" id="editable_text_id"
                                                name="editable_text_id[]"
                                                required>{{ $dataArray['editable_text'][$i]['value'] }}</textarea>
                                        </div>
                                    </div>
                    ...
                    <div class="form-group">
                        <h6>Encrypted</h6>
                        <div class="col-sm-20">
                            <select class="selectpicker form-control" data-style="btn-outline-primary" name="encrypted"
                                id="encrypted">
                        ...
                        <div id="encryption_field">
                            <div class="form-group">
                                ...
                                <input class="form-control" type="textname" id="encryption_key" name="encryption_key"
                                    value="{{ $dataArray['item']->encryption_key }}" required>
                            </div>
                        </div>
                    @else
...
<script>
    $(document).ready(function () {
        // Video Thumb validation and preview
        $('#video_thumb').change(function () {
            ...
            if (file) {
                ...
                reader.onload = function (e) {
                    const preview = '<div style="margin-top: 10px;"><img src="' + e.target.result + '" style="max-width: 200px; max-height: 200px; border-radius: 6px; border: 1px solid #e0e0e0;"><p style="font-size: 12px; color: #6c757d; margin-top: 5px;">New: ' + file.name + ' (' + (file.size / 1024).toFixed(2) + ' KB)</p></div>';
        ...
        $('#video_file').change(function () {
            ...
            if (file) {
                ...
                // Show preview
                const preview = '<div style="margin-top: 10px; padding: 10px; background: #f0f7ff; border-radius: 6px; border: 1px solid #0059b2;"><i class="fa fa-video-camera" style="color: #0059b2; margin-right: 8px;"></i><span style="font-size: 13px; color: #4a4a4a;">New: ' + file.name + ' (' + (file.size / (1024 * 1024)).toFixed(2) + ' MB)</span></div>';
