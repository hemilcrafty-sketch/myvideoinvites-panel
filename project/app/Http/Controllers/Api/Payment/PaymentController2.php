...
class PaymentController extends ApiController
{
        ...
        try {
            ...
            if (!$paymentGateway) {
                ...
                foreach ($datas['items'] as $value) {
                    if (MasterPurchaseHistory::whereTransactionId($value['id'])->exists())
                        continue;
            ...
            if (!$paymentGateway) {
                ...
                foreach ($datas['data'] as $value) {
                    ...
                    if (MasterPurchaseHistory::whereTransactionId($value['balance_transaction'])->exists())
                        continue;
    ...
    function getOrder(Request $request): array|string
    {
        ...
        if (!$user_data)
            return $this->failed(msg: "Invalid User");
        ...
        if ($order->gateway === 'razorpay') {
            ...
            try {
                $paymentGateway = PaymentGateway::initByGateway('stripe', null);
                if (!$paymentGateway)
                    return $this->failed(msg: "Stripe init failed");
                $stripe = $paymentGateway->client;
    ...
    {
        if (!$request->has('u') && $this->isFakeRequest($request))
            return $this->failed(msg: "Unauthorized");
    ...
    {
        if (!$request->has('u') && $this->isFakeRequestAndUser($request))
            return $this->failed(msg: "Unauthorized");
        ...
        if (empty($pmId) || empty($user) || empty($user->stripe_cus_id) || empty($billing_details) || empty($month) || empty($year))
            return $this->failed("Parameters missing!");
    ...
    function detachPm(Request $request): array|string
    {
        ...
        $pmId = $request->get('pm');
        $user = UserData::whereUid($this->uid)->first();
        if (empty($pmId) || empty($user) || empty($user->stripe_cus_id))
            return $this->failed("Parameters missing!");
        ...
        try {
            ...
            if ($customer->invoice_settings->default_payment_method === $pmId)
                return $this->failed(msg: "Cannot detach default payment method.");
    ...
    function createOrder(Request $request): array|string
    {
        ...
        if (DomainChecker::isEditor($request))
            $from = 'Editor';
        ...
        if ($refId) {
            $order = Order::whereCraftyId($refId)->first();
            if ($order)
                $this->uid = $order->user_id ?? $order->uid;
        }
        ...
        if ($name && $email && $number) {
            ...
            if (!$user_data) {
                ...
                $result = $userController->createFirebaseUser($request, $name, $email, $number, $password);
                if (!$result['success'])
                    return $this->sendRawResponse($result);
                $user_data = $result['data'];
            }
        } else {
            $user_data = UserData::where("uid", $this->uid)->first();
        ...
        if (!$paymentDetails['success'])
            return $this->sendRawResponse($paymentDetails);
        ...
        if (!$paymentGateway)
            return $this->failed(msg: "Gateway init failed");

        $gatewayType = $paymentGateway->name;

        $paymentGatewayRefId = $this->createUserPaymentGatewayRefId(user_data: $user_data, number: $number, paymentGateway: $paymentGateway);
        if (!$paymentGatewayRefId)
            return $this->failed(msg: "User error!");
        ...
        if (empty($datas) || is_string($datas))
            return $this->failed(msg: "Empty Data", datas: ['datas' => $datas, 'paymentDetails' => $paymentDetails, 'gatewayType' => $gatewayType]);
    ...
    private function getPaymentDetails(Request $request, UserData $user_data, $currency, $assetDetails, $code, $url, $seats): array
    {
        ...
        if ($tempAssetData && is_array($tempAssetData)) {
            ...
            if (!$assetDatas['success'])
                return $this->failed(msg: $assetDatas['message'], showDecoded: true);
        ...
        if (empty($payMode))
            return $this->failed(msg: 'Invalid Mode', showDecoded: true);
        if ($amount <= 0)
            return $this->failed(msg: 'Amount Error', showDecoded: true);
    ...
    private function cpm($amount, $code, $currency, $provideID = false): array
    {
        ...
        if ($currency === 'INR')
            $discount = round($amount * $promoData->disc / 100);
        else
            $discount = $amount * $promoData->disc / 100;

        if ($discountUpto !== 0)
            $discount = min($discount, $discountUpto);
    ...
    public function enterTransData(Request $request, $transaction_id, $method, $isManual, array|null $metaData = null): array
    {
        ...
        if (!$metaData['isSuccessed'])
            return ['success' => false, 'msg' => 'Payment not valid.', 'metaData' => $metaData];

        $user_data = $metaData['user_data'];
        $transaction_id = $metaData['transaction_id'];

        if (MasterPurchaseHistory::whereTransactionId($transaction_id)->exists())
            return $successRes;
        ...
        foreach ($assetDetails as $assetDetail) {
            ...
            if (strtoupper($currency_code) != "INR")
                $paidAmount = $assetDetail->usdVal * $exchangeRate;
            ...
            if (strtolower($paymentGateway->name) === 'stripe') {
                ...
                if ($transaction) {
                    ...
                    if (isset($transaction['source'])) {
                        ...
                        if (!empty($craftyId))
                            $query->orWhere('crafty_id', $craftyId);
    ...
    private function savePurchaseData(
        ...
        if (MasterPurchaseHistory::where('transaction_id', $transaction_id)->where('product_id', $assetDetail->id)->exists())
            return;
