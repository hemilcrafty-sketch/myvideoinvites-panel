...
//        if (!DomainChecker::isPanel($request)) return "error";
    ...
    public function enterTransData(Request $request, $transaction_id, $method, $isManual, array|null $metaData = null): array
    {
        ...
        foreach ($assetDetails as $assetDetail) {
            ...
            $errorMsg = $this->savePurchaseData(
                user_data: $user_data,
                contact: $contact,
...
//                    $isSuccessed = true;
    ...
    private function savePurchaseData(
        UserData $user_data,
                 ...
                 $fbcId,
                 $gclId,
                 $sales_person_id_for_report): string | null
    {


        if (MasterPurchaseHistory::where('transaction_id', $transaction_id)->where('product_id', $assetDetail->id)->exists()) return null;
