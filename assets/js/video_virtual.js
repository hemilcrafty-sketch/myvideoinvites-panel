console.log("Video Virtual Loaded - Version 2.0 - ASYNC EDIT FIX")

// Flag to track if event listeners have been attached
let moreTemplateListenersAttached = false;
document.addEventListener("DOMContentLoaded", function () {
    ...
    $('#api_virtual_modal').on('shown.bs.modal', function () {
        console.log("moreTemplateListenersAttachedmoreTemplateListenersAttached")
        if (!moreTemplateListenersAttached) {
            const moreTemplateVirtualContainer = document.getElementById("moreTemplateVirtualContainer");

            if (moreTemplateVirtualContainer) {
                // Only attach event listeners to elements inside moreTemplateVirtualContainer
                const columnSelects = moreTemplateVirtualContainer.getElementsByClassName('column');
                const operatorSelects = moreTemplateVirtualContainer.getElementsByClassName('operator');

                const saveButton = moreTemplateVirtualContainer.querySelector(".save-condition");
                if (saveButton) {
                ...
                resetQueryData(moreTemplateVirtualContainer);

                const addSortingButton = moreTemplateVirtualContainer.querySelector(".add-sorting");
                if (addSortingButton) {
                    addSortingButton.addEventListener("click", function (event) {
                        addSorting(event, moreTemplateVirtualContainer);
                    });
                }

                const addLimitButton = moreTemplateVirtualContainer.querySelector(".add-limit");
                if (addLimitButton) {
                    addLimitButton.addEventListener("click", function (event) {
                        addLimit(event, moreTemplateVirtualContainer);
                    });
                }

                moreTemplateListenersAttached = true;
            }
        }
    });
});
...
function resetQueryData(container) {
    if (!container) container = document.getElementById("moreTemplateVirtualContainer");
    const columnSelects = container.getElementsByClassName('column');
    Array.from(columnSelects).forEach(select => {
...
function handleColumnChange(selectElement, value = null, selectedOperator = null) {
    return new Promise((resolve, reject) => {
        ...
        let database = selectedOption.dataset.database || '';
        ...
        if (isDependent) {
            ...
            const fetchUrl = `${APP_BASE_URL}/get-options/${tableName}/${columnId}/${columnName}${databaseParam}`;
            ...
            if (columnType === 'string') {
                const databaseParam = database ? `/${database}` : '';
                fetch(`${APP_BASE_URL}/get-unique-options/Designs/${columnName}${databaseParam}`)
...
function saveCondition(event, parentElement) {
    ...
    if (["BETWEEN", "NOT BETWEEN"].includes(operator)) {
        ...
        if (multiSelect) {
            ...
            // For "=" and "!=" operators (single select), get value directly from dropdown
            if ((operator === "=" || operator === "!=") && !sortableContainer) {
                // Single select - get value directly from Select2
                const dropdownValue = $(multiSelect).val();

                if (dropdownValue) {
                    // Handle both single value and array
                    const valueArray = Array.isArray(dropdownValue) ? dropdownValue : [dropdownValue];
                    selectedValues = valueArray;
                    selectedTexts = valueArray.map(val => {
                        const option = Array.from(multiSelect.options).find(opt => opt.value == val);
                        // If option not found, use the value itself as fallback
                        return option ? option.text : `[ID:${val}]`;
                    });
                }
            } else if (sortableContainer && sortableContainer.querySelector('.sortable-item')) {
                // Multi-select with sortable - get values from sortable items
                const sortableItems = sortableContainer.querySelectorAll('.sortable-item');
                sortableItems.forEach(item => {
                    selectedValues.push(item.dataset.value);
                    // Get the text from the sortable item itself
                    const textSpan = item.querySelector('span:last-child');
                    if (textSpan) {
                        selectedTexts.push(textSpan.textContent.trim());
                    } else {
                        selectedTexts.push(item.dataset.value);
                    }
                });
            } else {
                // Fallback to dropdown order if no sortable container or sortable is empty
                const dropdownValue = $(multiSelect).val();

                if (dropdownValue) {
                    selectedValues = Array.isArray(dropdownValue) ? dropdownValue : [dropdownValue];
                    selectedTexts = selectedValues.map(val => {
                        const option = Array.from(multiSelect.options).find(opt => opt.value == val);
                        const text = option ? option.text : `[ID:${val}]`;
                        return text;
                    });
                }
            }
            ...
            const selectedValueTexts = selectedTexts;
...
async function editCondition(event, parentElement) {
    ...
    if (columnSelect && operatorSelect) {
        ...
        if (["BETWEEN", "NOT BETWEEN"].includes(operator)) {
            const valueFieldContainer = parentElement.querySelector(".value-container");
            const fromValue = valueFieldContainer.querySelector(".from-value");
            ...
            await handleColumnChange(columnSelect, value, operator);
        } else if (["LIKE %..%", "NOT LIKE %..%"].includes(operator)) {
...
// Process conditions for video virtual categories when editing CTA
async function processConditionsVideo(conditions, parentElement) {

    if (!parentElement) {
        parentElement = document.getElementById("moreTemplateVirtualContainer");
    }
    ...
    for (const condition of conditions) {
        ...
        console.log('Processing condition:', { column, operator, value });
        ...
        if (operator == "LIMIT") {
            ...
            let secondValue = value == "asc" ? "Ascending" : "Descending";
            setValueInTable(
                sortObject.column,
                column,
                operator,
            ...
            if (!columnObject) {
                console.warn('No columnObject found for column:', column);
                continue;
            }
                ...
                for (const pValue of processValue) {
                    try {
                        ...
                        const dynamicValue = await fetchOptions(
                            ...
                            pValue,
                            columnObject.database
                        );
                        resolvedValues.push(dynamicValue);
                    } catch (error) {
                        console.error('Error fetching option:', error);
                        resolvedValues.push(pValue);
                    }
                }
